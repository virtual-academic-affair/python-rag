...
from app.modules.rag.query.answering.pageindex import EMAIL_SYSTEM_PROMPT
from app.modules.rag.query.pipeline import RagQueryPipeline
from app.modules.rag.query.retrieval.retrieval_service import RetrievalContext
...
def _pipeline(analyzer=None, retrieval_context=None, faq_match=None):
    pipe = RagQueryPipeline()
    pipe._analyzer = analyzer or FakeAnalyzer()
    pipe._retrieval = SimpleNamespace(
        retrieve_context=AsyncMock(return_value=retrieval_context or RetrievalContext())
    )
    faq_answer = None
    if faq_match:
...
async def test_answer_chat_uses_common_engine_with_analyzer_enrollment_fallback():
    ...
    with patch("app.modules.rag.query.pipeline.run_agent_loop", agent_mock):
    ...
    pipe._retrieval.retrieve_context.assert_awaited_once_with(
    ...
    assert [s["type"] for s in result.steps] == ["query_analysis", "retrieval", "call"]
...
async def test_answer_chat_direct_reply_does_not_retrieve():
    ...
    pipe._retrieval.retrieve_context.assert_not_awaited()
...
async def test_answer_chat_uses_faq_match_without_reading_pageindex():
    ...
    assert [s["type"] for s in result.steps] == ["query_analysis", "retrieval", "faq_match"]
...
async def test_stream_chat_emits_common_pipeline_events_and_agent_events():
    ...
    async def fake_stream_agent_loop(**_kwargs):
    ...
    with patch("app.modules.rag.query.pipeline.stream_agent_loop", fake_stream_agent_loop):
        events = [
            event
...
async def test_stream_chat_uses_faq_pipeline_result_without_agent_events():
    ...
    assert [s["type"] for s in final["steps"]] == ["query_analysis", "retrieval", "faq_match"]
...
async def test_no_candidate_fallback_differs_between_chat_and_email():
    chat_pipe = _pipeline(analyzer=FakeAnalyzer(metadata_filter={}), retrieval_context=RetrievalContext())
    email_pipe = _pipeline(retrieval_context=RetrievalContext())
