...
from app.modules.rag.query.retrieval.rerank.cohere_reranker import CohereReranker
...
async def test_cohere_reranker_applies_valid_file_order():
    ...
    ranked = await CohereReranker(client=client).rerank_files("câu hỏi", _candidates())
...
async def test_cohere_reranker_falls_back_when_disabled_or_missing_key(monkeypatch):
    ...
    ranked = await CohereReranker(client=client).rerank_files("câu hỏi", original)
...
async def test_cohere_reranker_reranks_faqs_with_limit():
    ...
    ranked = await CohereReranker(client=client).rerank_faqs("câu hỏi", faq_docs, limit=2)
