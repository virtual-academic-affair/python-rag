...
class IngestEmailData(BaseModel):
    ...
    email_id: Optional[int] = Field(default=None, alias="emailId")
    message_id: Optional[int] = Field(default=None, alias="messageId")
    subject: Optional[str] = Field(default="")
    sender_email: Optional[str] = Field(default="", alias="senderEmail")
    sender_name: Optional[str] = Field(default="", alias="senderName")
    content: Optional[str] = Field(default="")

    @property
    def id(self) -> int:
        """Return emailId or messageId (whichever is present)."""
        value = self.email_id if self.email_id is not None else self.message_id
        if value is None:
            raise ValueError("IngestEmailData must have either emailId or messageId")
        return value
