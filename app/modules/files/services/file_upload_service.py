...
class FileUploadMixin:
    ...
    async def process_ocr_background(
        ...
        """Phase 1: Parse PDF via OCR to Markdown draft and mark awaiting_review."""
        ...
        try:
            processing_doc = await self.file_repo.claim_ocr_start(file_id)
            ...
            markdown_storage_path, page_count = await ingestion_service.parse_ocr_draft(
                ...
                file_path=file_path,
                original_storage_path=processing_doc.storage_path,
            )

            await_doc = await self.file_repo.mark_awaiting_review(
                file_id=file_id,
                markdown_storage_path=markdown_storage_path,
                page_count=page_count,
            )
            if not await_doc:
                raise NotFoundException("Processing file", file_id)
    ...
    async def process_indexing_background(
        self,
        file_id: str,
        display_name: str,
        progress_callback: Optional[Callable[[Dict[str, Any]], Awaitable[None]]] = None,
    ) -> None:
        """Phase 2: Index approved Markdown draft into PageIndex and Corpus."""
        start_bg = time.perf_counter()
        async def _notify(step: str, message: str):
            if progress_callback:
                await progress_callback({"step": step, "message": message, "file_id": file_id})
        ...
        try:
            ...
            ingest_result = await ingestion_service.index_approved_markdown(
                file_id=file_id,
                display_name=display_name,
                original_storage_path=file_doc.storage_path,
            )
            ...
            if not ready_doc:
                raise NotFoundException("File in processing state", file_id)
