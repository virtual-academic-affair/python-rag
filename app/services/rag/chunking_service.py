"""
Chunking Service (Sprint 2).

Builds semantic-ish chunks from parsed markdown pages.
Current strategy:
- Split by markdown headings first.
- Then split long sections by character window with overlap.
"""
...
from app.services.rag.llamaparse_ingest_service import ParsedMarkdownPage
            ...
            for part in self._window_split(text, chunk_size_chars, chunk_overlap_chars):
                chunks.append(
                    ChunkBlock(
                        ...
                        page_index_start=sec["page_start"],
                        page_index_end=sec["page_end"],
