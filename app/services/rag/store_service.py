...
class StoreService:
    ...
    async def delete_store(self, store_id: str, force: bool = False) -> bool:
        ...
        if file_count > 0:
            ...
            await self.file_repo.delete_by_store(store_id)
    ...
    async def sync_store_stats(self, store_id: str) -> StoreDocument:
        ...
        try:
            ...
            logger.info(f"Store stats synced: {store_name}")
    ...
    async def list_stores(self, skip: int = 0, limit: int = 100) -> tuple[list[StoreDocument], int]:
        """
        List stores with pagination.
        """
        docs, total = await self.store_repo.list_stores(skip, limit)
        return [StoreDocument(**d) for d in docs], total

    async def get_store(self, store_id: str) -> Optional[StoreDocument]:
        """
        Get store details by ID.
        """
        doc = await self.store_repo.find_by_id(store_id)
        return _to_store_model(doc)

    async def get_default_store(self) -> Optional[StoreDocument]:
        """
        Get the default store.
        """
        doc = await self.store_repo.find_default_store()
        return _to_store_model(doc)

    async def update_store(self, store_id: str, display_name: Optional[str] = None, is_default: Optional[bool] = None) -> StoreDocument:
        """
        ...
        if display_name:
            await self.store_repo.update_display_name(store_id, display_name)
